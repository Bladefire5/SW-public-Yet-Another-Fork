...
public sealed partial class TradingMenu : DefaultWindow
{
    ...
    private EntityUid? _selectedPreview;
    ...
    public TradingMenu()
    {
        ...
        ExamineButton.OnPressed += _ => ExamineSelected();
    ...
    private Control CreateCard(TradingMarketItemState item)
    {
        ...
        identity.AddChild(new Label
        {
            Text = $"Эталон: {item.StandardPrice}",
            HorizontalAlignment = HAlignment.Center,
        ...
        var buy = new Button
        {
            Text = item.LowestSellPrice is { } ask ? $"Купить {ask}" : "Купить —",
        ...
        var sell = new Button
        {
            Text = item.HighestBuyPrice is { } bid ? $"Продать {bid}" : "Продать —",
    ...
    private Texture? GetItemTexture(EntProtoId product)
    {
        if (!_prototypes.HasIndex(product))
            return null;
        return _entities.System<SpriteSystem>().GetPrototypeIcon(product).Default;
    }
    ...
    private Control CreateItemPreview(EntProtoId product, NetEntity? preview, Vector2 size)
    {
        ...
        {
            Texture = GetItemTexture(product),
            MinSize = size,
            SetSize = size,
    ...
    {
        if (_selectedPreview is not { } entity || !_entities.EntityExists(entity))
            return;
    ...
    {
        if (_state == null ||
            _hands.GetActiveHandEntity() is not { } held ||
